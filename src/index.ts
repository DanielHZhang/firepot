...
firepad.Firepad = (function(global) {
  let MonacoAdapter = firepad.MonacoAdapter;
  let FirebaseAdapter = firepad.FirebaseAdapter;
  let EditorClient = firepad.EditorClient;
  let EntityManager = firepad.EntityManager;
  let ATTR = firepad.AttributeConstants;
  let utils = firepad.utils;
  let LIST_TYPE = firepad.LineFormatting.LIST_TYPE;
  let CodeMirror = global.CodeMirror;
  let ace = global.ace;
  let monaco = global.monaco;
  ...
  function Firepad(ref, place, options) {
    ...
    if (CodeMirror && place instanceof CodeMirror) {
      ...
      let curValue = this.codeMirror_.getValue();
      if (curValue !== '') {
        throw new Error(
    ...
    this.firebaseAdapter_.on('ready', function() {
      ...
      let defaultText = self.getOption('defaultText', null);
