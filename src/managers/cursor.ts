...
/**
 * A cursor has a `position` and a `selectionEnd`. Both are zero-based indexes into the
 * document. When nothing is selected, `selectionEnd` is equal to `position`. When there
 * is a selection, `position` is always the side of the selection that would move if you
...
export class Cursor {
  ...
  public static fromJSON(obj: {position: number; selectionEnd: number}) {
